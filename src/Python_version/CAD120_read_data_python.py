...
if __name__ == '__main__':
    ...
    while(1):
        ...
        for current_frame in frames:
            ...
            for jj1,jj2 in zip([1,2,3,2,4,5,6,7,2,3],[2,3,4,4,5,12,7,13,6,6]):
                x1 = int(joints2D[current_frame][jj1]['position'][0])
                y1 = int(joints2D[current_frame][jj1]['position'][1])
                x2 = int(joints2D[current_frame][jj2]['position'][0])
                y2 = int(joints2D[current_frame][jj2]['position'][1])
                cv2.line(img_rgb,(x1,y1),(x2,y2),(5,255,10),3)
            for jj in [1,2,3,4,5,6,7,12,13,14,15]:
                x2 = int(joints2D[current_frame][jj]['position'][0])
                y2 = int(joints2D[current_frame][jj]['position'][1])
                cv2.circle(img_rgb,(x2,y2),9,(5,10,255),-1)
            x_world = []
            y_world = []
            z_world = []
            ...
            for j in range(len(obj_name)):
                ...
                x2 = int(corners[2])
                y2 = int(corners[3])
                cv2.line(img_rgb,(x1,y1),(x2,y1),(5,10,255),2)
                cv2.line(img_rgb,(x1,y2),(x2,y2),(5,10,255),2)
                cv2.line(img_rgb,(x1,y1),(x1,y2),(5,10,255),2)
                cv2.line(img_rgb,(x2,y1),(x2,y2),(5,10,255),2)
