...
if __name__ == '__main__':
    ...
    for person in ['1','3','4','5']:
        for activity in activity_list:
            for person_id_counter in range(len(person_id_list[person][activity])):
                f_act2 = open('/home/omari/Python/cad120/src/Level2_act_data/person'+person+'/'+activity.split('/')[0]+'_'+person_id_list[person][activity][person_id_counter]+'.txt', 'r')
                for count,line in enumerate(f_act2):
                    if count==0:
                        pass
                    else:
                        header = line.split(':')[0]
                        if header not in Headers:
                            Headers[header] = {}
                            Headers[header]['approach'] = 0
                            Headers[header]['depart'] = 0
                f_act2.close()
    KEYS = Headers.keys()
    KEYS_NAMES = ''
    for k in range(len(KEYS)-1):
        key = '-'.join(KEYS[k].split(','))
        KEYS_NAMES += key+'-approach,'+key+'-depart,'
    key = '-'.join(KEYS[-1].split(','))
    KEYS_NAMES+= key+'-approach,'+key+'-depart\n'
    ...
    #             for k in range(len(KEYS)-1):
    #                 key = KEYS[k]
    #                 for t in ['approach','depart']:
    #                     Histogram += str(H[key][t])+','
    #             Histogram += str(H[KEYS[-1]]['approach'])+','+str(H[KEYS[-1]]['depart'])+'\n'
    #             f_Hist.write(Histogram)
    #             f_act2.close()
    #         f_Hist.close()
    for person in ['1','3','4','5']:
        for s in range(3):
            f_Stream = open('/home/omari/Python/cad120/src/Long_streams/stream'+person+'_'+str(s)+'.txt', 'w')
            Ground_truth = {}
            for activity in activity_list:
                exist = 0
                f_qsr = open('/home/omari/Python/cad120/src/QSR_data/person'+person+'/'+activity.split('/')[0]+'_'+person_id_list[person][activity][s]+'.txt', 'r')
                for count,line in enumerate(f_qsr):
                    if count>0:
                        ...
                        if header == key:
                            Long_str[key] += data+','
                            exist = 1
                Ground_truth[activity] = len(data.split(','))
                print activity,len(data.split(','))
            print '------------'
            Long_str = {}
            for key in KEYS:
                Long_str[key] = ''
            ...
            frame = 0
            for activity in activity_list:
                f_Stream.write(activity.split('/')[0]+':'+str(frame)+','+str(Ground_truth[activity]+frame)+'\n')
                frame += Ground_truth[activity]

            for key in KEYS:
                f_Stream.write(key+':'+','.join(Long_str[key].split(',')[:-1])+'\n')
            f_Stream.close()
