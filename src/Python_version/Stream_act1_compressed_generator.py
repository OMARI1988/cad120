...
if __name__ == '__main__':
    ...
    for person in ['1','3','4','5']:
        for s in range(1):
            f_Stream1 = open('/home/omari/Python/cad120/src/Long_streams/act1_stream'+person+'_'+str(s)+'.txt', 'r')
            f_Stream2 = open('/home/omari/Python/cad120/src/Long_streams/compressed_stream'+person+'_'+str(s)+'.txt', 'w')
            for count,line in enumerate(f_Stream1):
                if count == 7:
                    qsrs = np.empty((29,len(line.split(':')[1].split('\n')[0].split(','))))
                if count<7:
                    f_Stream2.write(line)
                else:
                    header = line.split(':')[0]
            ...
            counter = 1
            for n in range(1,len(line.split(':')[1].split('\n')[0].split(','))):
                if np.sum(qsrs[:,n])>0.5:
                    counter += 1

            qsrs_f = np.zeros((29,counter))
            counter = 1
            frames = ['0']
            for n in range(1,len(line.split(':')[1].split('\n')[0].split(','))):
                if np.sum(qsrs[:,n])>0.5:
                    qsrs_f[:,counter] = qsrs[:,n]
                    counter += 1
                    frames.append(str(n))

            f_Stream2.write('frames:'+','.join(frames)+'\n')
            for n in range(29):
                A = []
                for l in qsrs_f[n,:]:
                    A.append(str(int(l)))
                f_Stream2.write(KEYS[n]+':'+','.join(A)+'\n')
