...
if __name__ == '__main__':
    ...
    Histograms = {}
    for activity in activity_list:
        f_Hist = open('/home/omari/Python/cad120/src/Level2_act_data/Histogram/'+activity.split('/')[0]+'.txt', 'r')
        Histograms[activity] = []
        for count,line in enumerate(f_Hist):
            if count > 0:
                A = np.zeros((29*2))
                for c,i in enumerate(line.split('\n')[0].split(',')):
                    A[c] = int(i)
    ...
    for person in ['5']:
        for s in range(3):
            ...
            for count,line in enumerate(f_Stream1):
                if count == 7:
                    frames = len(line.split(':')[1].split('\n')[0].split(','))
                    actual_frames = line.split(':')[1].split('\n')[0].split(',')
                    qsrs = np.empty((29,frames))
                    f_Stream2.write('Results\n')
                if count<7:
                    ...
                    qsrs[count-8,:] = data
            for c1 in range(2,70):
                for f in range(frames-c1):
                    # print f,f+c1
                    Histogram = np.zeros((29*2))
                    A = qsrs[:,:]
                    for n1 in range(29):
                        B = A[n1,f:f+c1+1]
                        for val1 in B:
