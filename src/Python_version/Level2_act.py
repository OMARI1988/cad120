    ...
    for person in ['1','3','4','5']:
        ...
        for activity in activity_list:
            for person_id_counter in range(len(person_id_list[person][activity])):
                f_act1 = open('/home/omari/Python/cad120/src/Level1_act_data/person'+person+'/'+activity.split('/')[0]+'_'+person_id_list[person][activity][person_id_counter]+'.txt', 'r')
                f_act2 = open('/home/omari/Python/cad120/src/Level2_act_data/person'+person+'/'+activity.split('/')[0]+'_'+person_id_list[person][activity][person_id_counter]+'.txt', 'w')
                for count,line in enumerate(f_act1):
                    Act2 = ['0']
                    if count==0:
                        f_act2.write(line)
                    else:
                        ...
                        for t,d in enumerate(data):
                            if d == '1':
                                f_act2.write(header+':approach'+','+str(t)+'\n')
                            if d == '2':
                                f_act2.write(header+':depart'+','+str(t)+'\n')
                f_act1.close()
                f_act2.close()
